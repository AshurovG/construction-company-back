...
class QuestionsReoisitory {
  static insertNew(title, text) {
    return new Promise((resolve, reject) => {
      db.query(
        "INSERT INTO questions(questions_title, questions_text) VALUES ($1, $2) RETURNING *",
        [title, text],
        (error, results) => {
          if (error) {
            reject(error);
  ...
  static updateById(id, title, text) {
    return new Promise((resolve, reject) => {
      db.query(
        "UPDATE questions set questions_title = $1, questions_text = $2 where questions_id = $3 RETURNING *",
        [title, text, id],
        (error, results) => {
          if (error) {
            reject(error);
