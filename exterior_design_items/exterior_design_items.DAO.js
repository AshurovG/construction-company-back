...
} = require("./exterior_design_items.repository");
const fs = require("fs");
...
class ExteriorDesignItemsDAO {
  ...
  static async _validate(exteriorDesign) {
    ...
    if (
      await (exteriorDesign.url === undefined ||
  ...
  static async isExistsIdOneAndIdMany(idOne, idMany) {
    ...
    if (
      (await ExteriorDesignItemsRepository.getByIdOneToMany(idOne, idMany)) ===
      undefined
    ) {
      let error = new Error(
  ...
  static async deleteById(idOne, idMany) {
    try {
      ...
      const query = await ExteriorDesignItemsRepository.deleteById(
        idOne,
        idMany
      );
      return query;
